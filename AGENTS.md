...
## Test plan
- [ ] the merge gate is green (required); `make signoff` ran on the workstation for a change that touches the chart or the edge (deploy ADR-0012)
...
`pr-title-lint` now fails a PR body that uses the bare form (.github#306), so the merge cannot leave the issue open.
...
Codified in `ADR-0007`.
Read the ADR for the reasoning; this section is the operational contract.

**The merge gate is a fixed, ten-minute set** (deploy ADR-0012, 2026-08-25):
formatting, lint, unit tests, render guards, secret/RBAC/registry guards,
CodeQL, govulncheck. Exit tests, cluster installs and restore drills never run
on `pull_request`; they run on `main` and on a schedule and feed the launch
scorecard. A required job that cannot finish inside ten minutes leaves the set.
...
**Full pre-merge checklist:** the four conditions above are the mechanical part; the complete canonical list (walker gates, CodeQL findings, coverage delta, ADR + trap obligations, escape-hatch ban, `Docs-PR:` trailer) lives at ``docs/agents/pr-checklist.md`` (docs → `agents/pr-checklist.md`) in the workspace docs repo. Single source of truth — do not restate items here.
...
The open-core boundary is tracked in the ADR series, which lives in the
**local-only** `docs` repository (deleted from GitHub 2026-09-05), and
the migration map at `architecture/open-core/MIGRATION-MAP.md`:

- `ADR-0050` — Open-core licensing boundary (Apache vs ELv2 vs closed vs private)
- `ADR-0053` — Retire `platform-sdk` and `platform-clients`; fold into the `gibson` monorepo
- `ADR-0056` — Repo-topology consolidation (operators, ext-authz, spiffe-jwks-exporter into `gibson`)
- `ADR-0058` — The public SDK is the component-dev surface only (admin protos move daemon-local)
...
## 12. Writing rules for anything a human reads

Every document an agent produces for zeroroot is written in Simplified
Technical English (ASD-STE100). That covers docs pages, READMEs, ADRs, PR
descriptions, issue bodies, runbooks, error messages, and UI copy. Owner
call, 2026-08-25. `docs-site` enforces the machine-checkable part with
`scripts/check-ste-prose.mjs`.

Two modes:
- strict: procedures, runbooks, safety text, error messages. Every rule,
  both length caps.
- STE-flavored: general prose (READMEs, PR descriptions, docs). Sentence,
  paragraph, active-voice and verb discipline. The word list relaxes so the
...
- No stacked auxiliaries. Write "this improves X".
- No "-ing" main verb where a simple tense works.

Sentences:
- One instruction per sentence. At most 20 words for an instruction, 25 for
...
Before you finish, check the text:
