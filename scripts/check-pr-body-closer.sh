...
if [ "${1:-}" = "--selftest" ]; then
  ...
  printf 'Closes charts#12\n' > "$t/bad1"; printf 'Fixes hosted#3 and more\n' > "$t/bad2"
  printf 'Closes #12\n' > "$t/ok1"; printf 'Closes zeroroot-ai/charts#12\n' > "$t/ok2"; printf 'Closes https://github.com/zeroroot-ai/charts/issues/12\n' > "$t/ok3"; printf 'Refs charts#12 (not a closer)\n' > "$t/ok4"
