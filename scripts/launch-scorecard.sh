...
# --- Reference limits shown next to the behaviour metrics below. ---------------
# These annotate the numbers with a ❌/✅ marker. They are context, not orders.
...
collect() {
  ...
  # GITHUB_TOKEN sees PUBLIC repos only. If a private repo is invisible, every
  # private-repo number below is silently wrong — and a silently wrong
  # scorecard is worse than no scorecard.
  #
  # The canary was `gitops`, which was DELETED on 2026-09-04 (ADR-0086). The
  # guard then fired on every run and the board stopped being produced at all.
  # `hosted` replaces it: private, long-lived, and the repo that owns four of
  # the six blocker workflows, so if it is invisible the board is worthless.
  if ! gh api "orgs/${ORG}/repos" --paginate --jq '.[].name' 2>/dev/null | grep -qx 'hosted'; then
    die "cannot see private repos (hosted absent) — set GH_PAT_PLATFORM_RO; a partial scorecard is not allowed"
  fi
...
render() {
  ...
  if [ -n "${PREV_BODY:-}" ] && [ -f "${PREV_BODY}" ]; then
    ...
    # dispatch, a re-run). Keep the newest value for each day; today's own
    # earlier lines are dropped here and re-added below.
  ...
  # --- Verdict: green when every blocker passes on main. ---------------------
  local verdict="GREEN"
  [ "$green" -lt "$total" ] && verdict="RED"
...
This board reports measured state. It gives no orders.
...
## Other launch signals

Outcomes that gate a launch surface but are not one of the six blockers.
Measured the same way; never counted in the verdict above.
